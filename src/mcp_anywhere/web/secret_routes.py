...
class SecretFileUploadData(BaseModel):
    ...
    def validate_env_var_name(cls, v):
        ...
        if not re.match(r"^[A-Z][A-Z0-9_]*$", v):
            raise ValueError(
                "Environment variable name must start with a letter and contain only uppercase letters, numbers, and underscores"
            )
        return v
...
async def upload_secret_file(request: Request) -> Response:
    ...
    try:
        ...
        is_valid, error_msg = file_manager.validate_file(
            uploaded_file.filename, file_size
        )
        ...
        try:
            ...
            return JSONResponse(
                {"error": f"Validation error: {e.errors()[0]['msg']}"}, status_code=400
            )
        ...
        stored_filename = file_manager.store_file(
            server_id, uploaded_file.filename, file_content
        )
        ...
        logger.info(
            f"Uploaded secret file for server {server_id}: {uploaded_file.filename}"
        )

        # Return success response
        if request.headers.get("HX-Request"):
            # HTMX request - return partial HTML with updated secret files list
            # Refresh the server data to get the updated secret files list
            async with get_async_session() as db_session:
                stmt = (
                    select(MCPServer)
            ...
            return templates.TemplateResponse(
                request,
                "partials/secret_files_list.html",
        ...
        return JSONResponse(
            {"error": f"Failed to upload file: {str(e)}"}, status_code=500
        )
...
async def list_secret_files(request: Request) -> JSONResponse:
    ...
    try:
        async with get_async_session() as db_session:
            ...
            return JSONResponse({"server_id": server_id, "secret_files": secret_files})

    except Exception as e:
        logger.exception(f"Error listing secret files for server {server_id}")
        return JSONResponse(
            {"error": f"Failed to list files: {str(e)}"}, status_code=500
        )
...
async def delete_secret_file(request: Request) -> Response:
    ...
    try:
        async with get_async_session() as db_session:
            ...
            if not secret_file:
                return JSONResponse(
                    {"error": f"Secret file '{file_id}' not found"}, status_code=404
                )
        ...
        if request.headers.get("HX-Request"):
            ...
            # Regular request - return JSON
            return JSONResponse(
                {"success": True, "message": "Secret file deleted successfully"}
            )
...
secret_file_routes = [
    ...
    Route(
        "/servers/{server_id}/secrets/{file_id}", delete_secret_file, methods=["DELETE"]
    ),
]
