...
class SecureFileManager:
    ...
    def __init__(self, storage_path: Path | None = None) -> None:
        ...
        self.storage_path = storage_path or Config.SECRETS_DIR
    ...
    def store_file(
        self, server_id: str, original_filename: str, file_content: bytes
    ) -> str:
        ...
        logger.info(
            f"Stored secret file for server {server_id}: {original_filename} -> {stored_filename}"
        )
    ...
    def delete_file(self, server_id: str, stored_filename: str) -> bool:
        ...
        if file_path.exists():
            file_path.unlink()
            logger.info(
                f"Deleted secret file for server {server_id}: {stored_filename}"
            )
            return True
        return False
    ...
    def prepare_container_files(
        self, server_id: str, secret_files: list
    ) -> dict[str, str]:
